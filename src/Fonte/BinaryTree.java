...
public class BinaryTree {
    ...
    public void inserir(Node node, String valor) {
        if(this.root == null){
    ...
    public Node remover(String valor) throws Exception{
        return remover(this.root, valor);
    }
    
    private Node remover(Node node, String valor) throws Exception{
        if(this.root == null){
    ...
    public Node busca (String k) {
        return busca(this.root,k);
}
    
    private Node busca (Node root, String k) {
    if (root == null || root.getValor().equals(k))
       return root;
    if (root.getValor().compareTo(k) > 0)
       return busca (root.getNoEsquerda(), k);
    else
       return busca (root.getNoDireita(), k);
}
}